...
android {
    ...
    signingConfigs {
        create("debugShared") {
            ...
            // If missing, the build will use the default system debug keystore.
    ...
    buildTypes {
        debug {
            // Use the shared keystore for consistent SHA-1 across machines
...
dependencies {
    ...
    // Excludes the ads-identifier module: it's the actual code that reads the
    // Google Advertising ID, and its mere presence in the APK is what Play
    // Console's SDK scanner flags - the manifest metadata flag above only stops
    // Analytics from calling it, it doesn't remove the capability from the binary.
