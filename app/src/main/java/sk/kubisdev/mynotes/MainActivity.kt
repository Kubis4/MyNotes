...
import sk.kubisdev.mynotes.auth.AuthenticationScreen
import sk.kubisdev.mynotes.backup.BackupWorker
import sk.kubisdev.mynotes.backup.BackupFrequency
import sk.kubisdev.mynotes.settings.LocaleManager
import sk.kubisdev.mynotes.settings.SettingsRepository
import sk.kubisdev.mynotes.settings.SettingsViewModel
import sk.kubisdev.mynotes.settings.model.AppTheme
import sk.kubisdev.mynotes.ui.components.LocalGradientHeaders
import sk.kubisdev.mynotes.ui.screens.OnboardingScreen
import sk.kubisdev.mynotes.ui.theme.MyNotesDynamicTheme
...
class MainActivity : FragmentActivity() {
    ...
    // Deep-link target from a tapped reminder notification. Compose state so
    // AppNavigation reacts to it; -1 means "nothing pending".
    private var pendingReminderNoteId by mutableStateOf(-1)
    private var pendingReminderNoteType by mutableStateOf("TEXT")

    companion object {
        const val EXTRA_OPEN_NOTE_ID = "open_note_id"
        const val EXTRA_OPEN_NOTE_TYPE = "open_note_type"
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // A reminder notification may have launched us with a note to open.
                    ...
                    if (authRequired && !isAuthenticated) {
                        ...
                        if (!settings.hasCompletedOnboarding) {
                            ...
                            AppNavigation(
                                ...
                                settingsViewModel = settingsViewModel,
                                deepLinkNoteId = pendingReminderNoteId,
                                deepLinkNoteType = pendingReminderNoteType,
                                onDeepLinkHandled = { pendingReminderNoteId = -1 }
                            )
                        }
                    }
                }
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        // Keep getIntent() in sync and route the reminder tap while already running.
        setIntent(intent)
        handleReminderIntent(intent)
    }
    ...
    private fun handleReminderIntent(intent: Intent?) {
        ...
        val noteId = intent.getIntExtra(EXTRA_OPEN_NOTE_ID, -1)
        if (noteId > 0) {
            pendingReminderNoteId = noteId
            pendingReminderNoteType = intent.getStringExtra(EXTRA_OPEN_NOTE_TYPE) ?: "TEXT"
        }
    }
