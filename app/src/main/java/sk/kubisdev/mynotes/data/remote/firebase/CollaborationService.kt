...
import sk.kubisdev.mynotes.data.remote.models.*
    ...
    fun getCollaborativeNoteFlow(noteId: String): Flow<CollaborativeNote?> = callbackFlow {
        ...
        val listener = notesCollection.document(noteId)
            .addSnapshotListener { snapshot, error ->
                if (error != null) {
                    // PERMISSION_DENIED is the expected outcome right after this user
                    // leaves/is removed from the note (or the owner deletes it): the
                    // security rules correctly reject the listener the instant
                    // membership changes, which can land before the editor screen has
                    // finished tearing down its own LaunchedEffect. close(error) used to
                    // rethrow that into the collector, crashing the app on exactly the
                    // "leave a shared note" path. Emitting null instead lets callers
                    // treat it the same as "note not found" and keeps the flow alive.
                    // Logged at debug (not error) level for that expected case - it's
                    // not something gone wrong, just this listener's normal end of life.
                    if (error.code == com.google.firebase.firestore.FirebaseFirestoreException.Code.PERMISSION_DENIED) {
                        Log.d(TAG, "Listener for note $noteId ended: no longer a member (left/removed/deleted)")
                    } else {
